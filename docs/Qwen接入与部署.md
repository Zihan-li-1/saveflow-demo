...
需要 Node.js 22+。在 `saveflow-demo` 目录执行：
...
- “餐饮消费自动存5%。” → 提取餐饮和比例，月金额沿用演示默认2500元并明确显示。
...
- `scripts/dev-qwen.mjs`：本地同源服务，监听回环地址，同时启动 Next 和 `/api/agent`，本地每分钟最多10次请求。
...
- “目标总额”按模拟已储蓄2400元计算剩余差额；月份为演示计算口径，包含当前月。未指定比例时为0，不自动推断比例。当前草稿不保存正式到期日期，也不保证达到目标。
...
当前没有 API Key，自动化测试使用假的供应商响应，不产生模型费用，也不能替代真实账号联调。完成环境变量后，首次用一条合成指令验证返回和 Token，再逐步扩大测试。
