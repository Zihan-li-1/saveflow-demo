...
const server = http.createServer(async (req, res) => {
  if (req.url?.split('?')[0] !== '/api/agent') return nextHandler(req, res);
  const now = Date.now();
  while (visits.length && visits[0] < now - 60000) visits.shift();
  if (visits.length >= 10) { res.writeHead(429, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ code: 'RATE_LIMIT', message: '每分钟最多10次请求，请稍后再试。' })); return; }
  visits.push(now);
  try {
    ...
    const response = await handleAgent(request);
