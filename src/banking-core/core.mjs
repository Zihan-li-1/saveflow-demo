 ...
 * @param {{source?: Parameters<typeof createFinancialContext>[0], now?: () => number, store?: OperationStore, policy?: {version: string, maxTransferFen: number, previewTtlMs: number}}} [options] */
export function createBankingCore(options = {}) {
  const { repository, commitTransfer, ownerId } = createFinancialContext(options.source);
  ...
  function guard(run, operationId) {
    try { return { ok: true, data: run() }; }
  ...
  function riskCheck(input) {
    const result = guard(() => {
      validateTransfer(input);
      const account = repository.getAccount(input.fromAccountId);
      const payee = repository.getPayee(input.payeeId);
  ...
  function fail(record, error) {
    record.error = error;
    if (error.uncertain) { record.state = 'unknown'; save(record); }
    else { record.receipt = receiptFor(record, 'failed', error.message); move(record, 'FAILED'); }
  }
  ...
  function recheck(record) {
    ...
    const risk = riskCheck(record.request.input);
    if (!risk.allowed && risk.error) throw new BankingError(risk.error.code, risk.error.message);
    if (risk.policyVersion !== record.risk?.policyVersion || canonical(record.preview.exactEffects[0]) !== canonical(effectFor(record.request.input))) throw new BankingError('PREVIEW_STALE', '账户、收款人或风险条件已变化，请重新预览并确认');
  }
  ...
  return Object.freeze({
    ...
    async prepare(request) {
      ...
      try {
        ...
        save(record); move(record, 'PREPARED');
        const risk = riskCheck(input); record.risk = risk;
        if (!risk.allowed && risk.error) throw new BankingError(risk.error.code, risk.error.message);
        const effect = effectFor(input);
        const expiresAt = new Date(now() + policy.previewTtlMs).toISOString();
        const contextSnapshotId = repository.getContextInfo().snapshotId;
    ...
    decide(operationId, decision) {
      return guard(() => {
        const record = lookup(operationId);
        ...
        if (decision.decision === 'confirm') {
          try { recheck(record); } catch (error) { fail(record, errorData(error)); throw error; }
        }
        ...
        move(record, decision.decision === 'confirm' ? 'CONFIRM' : 'CANCEL');
    ...
    execute(operationId, previewHash) {
      return guard(() => {
        ...
        try {
          recheck(record); move(record, 'EXECUTE');
          // No async boundary: within one Mock runtime the ledger + receipt commit is indivisible.
          const transaction = commitTransfer(record.preview.exactEffects[0], operationId, new Date(now()).toISOString());
          record.receipt = receiptFor(record, 'succeeded', '模拟转账成功', [transaction.id]);
          move(record, 'SUCCEEDED');
          return record.receipt;
        } catch (error) { fail(record, errorData(error)); throw error; }
      }, operationId);
    },
    ...
    getOperation(operationId) {
      return guard(() => {
        ...
        const record = store.get(ownerId, operationId);
    ...
    createLegacyPlan(operationId, input) {
      assertId(operationId);
      if (input.confirmed !== true) throw new BankingError('CONFIRMATION_REQUIRED', '操作前需要明确确认');
      assertFen(input.monthlySavingFen);
      if (input.monthlySavingFen > repository.getDemoSettings().monthlySavingCapFen || !Number.isInteger(input.saveRateBps) || Number(input.saveRateBps) < 0 || Number(input.saveRateBps) > 10000) throw new BankingError('VALIDATION_ERROR', '月金额或储蓄比例超出模拟范围');
      const fingerprint = store.fingerprint('legacy.create-plan', input);
      const existing = store.get(ownerId, operationId);
      if (existing) {
      ...
      save(record); return structuredClone(record.receipt);
    },
  });
}
