...
export async function mockRequest<A extends Action>(action: A, input: RequestMap[A], operationId: string): Promise<ResultMap[A]> {
  try { return await legacyRequest(action, input, operationId) as ResultMap[A]; }
  catch (error) {
